...
class MyGUI():
    ...
    def launch_query(self,number):
        ...
        match number:
            case 1:
                description = """La liste des noms commerciaux de médicaments correspondant à un nom en DCI,\n
                                classés par ordre alphabétique et taille de conditionnement."""
                entry1.insert(0, "DCI")
                entry1.bind("<FocusIn>", lambda event, arg="DCI": self.clear_default_entry(event, arg))
                
                entriesToShow.append(entry1)
            case 2:
                description = """La liste des pathologies qui peuvent être prise en charge 
                ...
                entriesToShow.append(entry1)
            case 3:
                description = """La spécialité de médecins pour laquelle les médecins
                                    prescrivent le plus de médicaments"""
            case 4:
                description = """Tous les utilisateurs ayant consommé un médicament spécifique
            ...
            case 5:
                description = """Tous les  patients ayant été traités par un médicament (sous sa DCI)
                ...
                entriesToShow.append(entry1)
            case 6:
                description = """La liste des médecins ayant prescrit des médicaments
                                    ne relevant pas de leur spécialité"""
            case 7:
                description = """Pour chaque décennie entre 1950 et 2020,(1950−59,1960−69,...),
                                    le médicament le plus consommé
                                    par des patients nés durant cette décennie"""
                entry1.insert(0, "Date de naissance (YYYY-MM-DD)")
                entry1.bind("<FocusIn>", lambda event, arg="Date de naissance (YYYY-MM-DD)": self.clear_default_entry(event, arg))
                entry2.insert(0, "Date de naissance (YYYY-MM-DD)")
                entry2.bind("<FocusIn>", lambda event, arg="Date de naissance (YYYY-MM-DD)": self.clear_default_entry(event, arg))
        ...
        text = tk.Label(new_window, text=description,pady=20)
        text.pack()
    ...
    def launch_quary(self,number,cursor,args):
        ...
        text = tk.Label(result_window, text=result,pady=20)
        text.pack()
